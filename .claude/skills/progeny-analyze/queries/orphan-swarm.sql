...
--    pg-oo metrics "SELECT ROUND(MAX(value)) AS peak FROM \"progeny_system_orphan_max_comm_count\" GROUP BY __name__"
--    A jump into the dozens = a swarm forming.
--
-- 2) NAMED swarms (log record, emitted only when a cluster >= PROGENY_SWARM_THRESHOLD,
--    default 10). Each has comm, count, sample PIDs to kill, and an ancestry.
--    pg-oo logs "<this>"
--
